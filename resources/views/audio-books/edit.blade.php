...
@section('content')
    <div class="mb-6">
        <a href="/katalog-audio/{{ $audioBook->id }}" class="text-slate-600 font-medium text-sm inline-flex items-center gap-2">
            &larr; Kembali ke Detail Buku
        </a>
    </div>

    <div class="card bg-white border border-black/10 shadow-xl max-w-lg mx-auto p-6 sm:p-8">
        <h2 class="text-2xl font-bold text-black mb-6">Edit Buku</h2>
...
{{ $errors->first() }}
        ...
        <form method="POST" action="/katalog-audio/{{ $audioBook->id }}" enctype="multipart/form-data">
            ...
            <div class="form-control w-full mb-4">
                ...
                <input id="title" type="text" name="title" value="{{ old('title', $audioBook->judul) }}" class="input input-bordered w-full bg-white text-black border-black/15" required>
            </div>

            <!-- Import description from .txt file -->
            <div class="form-control w-full mb-4">
                <span class="label-text text-slate-600 text-sm font-medium mb-2 block">Ganti Deskripsi via File Teks (.txt)</span>
                <label for="description_file" class="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-black/15 rounded-xl p-8 sm:p-10 text-center bg-black/[0.03] cursor-pointer hover:border-[#b8860b] hover:bg-[#b8860b]/10 transition-all duration-200 w-full" id="dropzone" style="padding: 1.2rem;">
            ...
            <!-- Description field -->
            <div class="form-control w-full mb-4">
                <label for="description" class="label-text text-slate-600 text-sm font-medium mb-2 block">Deskripsi / Isi Buku</label>
                <textarea id="description" name="description" rows="8" class="textarea textarea-bordered w-full bg-white text-black border-black/15" placeholder="Tulis atau tempel deskripsi buku di sini...">{{ old('description', $audioBook->deskripsi) }}</textarea>
            </div>
